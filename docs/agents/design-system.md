...
- **Usage Sync settings** use `.usage-window-row` /
  `.usage-window-label` / `.usage-window-bar` for account-backed provider
  session and weekly limits, with native progress elements rather than inline
  width styles. `.usage-sharing-summary` / `.usage-sharing-block` carry the
  exact aggregate allowlist and exclusion copy, `.usage-attribution` identifies
  account versus private-installation attribution, and `.usage-source-section` /
  `.usage-source-row` / `.usage-source-state` provide compact independent source
  status. A failed or pending source exposes the native `Retry sync` action;
  source diagnostics must be fixed privacy-safe classifications and never raw
  parser text or paths.
