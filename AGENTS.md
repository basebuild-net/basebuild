...
16. **Release builds embed the frontend.** Never distribute a `tauri dev` / dev-server binary. Public releases go through the cross-platform `tauri build` matrix in [`.github/workflows/ci-release.yml`](./.github/workflows/ci-release.yml) — a binary that loads the UI from `127.0.0.1:1420` (`ERR_CONNECTION_REFUSED`) is a dev build and must never ship. CI enforces the shared production config on every PR, probes the packaged Windows webview, and verifies that the draft contains Windows, Linux, macOS, and signed updater artifacts. See [DEVELOPMENT.md](./docs/DEVELOPMENT.md#release-build-integrity-guards).
