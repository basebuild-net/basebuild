...
16. **Release builds embed the frontend.** Never distribute a `tauri dev` / dev-server binary. Public releases go through the `tauri build` pipeline in [`.github/workflows/windows.yml`](./.github/workflows/windows.yml) — a binary that loads the UI from `127.0.0.1:1420` (`ERR_CONNECTION_REFUSED`) is a dev build and must never ship. Two CI guards enforce this: `npm run check:release-config` (every PR) and the release job's webview probe (`scripts/verify-prod-webview.mjs`). See [DEVELOPMENT.md](./docs/DEVELOPMENT.md#release-build-integrity-guards).
