...
pub struct NativeModel {
    ...
    /// Wire-protocol kind from the model catalog: `openai-completions`,
    ...
    pub api_kind: String,
    /// The model's API base URL from the model catalog (e.g.
    /// `https://server.codeium.com` for devin). Empty for legacy rows and
    /// for providers whose base URL is credential-driven.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub base_url: String,
    /// Per-million-token input cost (USD), from the model catalog. Null when
