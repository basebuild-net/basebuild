...
- [x] 3.1 Migration: `pending_interactions` table (id, session_id, run_id,
...
- [x] 3.2 Register `ask_user` in `tool_runtime_service.rs` (questions schema:
...
- [x] 3.3 `agent_loop_service.rs`: on `ask_user`, persist the interaction,
...
- [x] 3.4 Commands `native_interaction_resolve` / `native_interaction_pending`
...
- [x] 4.1 `QuestionCard.tsx` rendered from tool events in `ChatPanel.tsx`:
...
- [x] 4.2 Composer answer routing in `ChatPanel.tsx`: single pending
      text/free-text question captures the next send with a visible
      "answering" indicator + send-as-message escape.
- [x] 4.3 Persist question/answer as tool-event rows so history reloads
      answered cards; provider history carries answers as tool results only.
- [x] 4.4 Update `skills/basebuild-planning/SKILL.md` (+ templates if needed)
...
- [x] 4.5 Pipeline turns (`pipeline_service.rs`) expose `ask_user` to
      generation/wizard turns so skill-driven picking loops work in-app.
- [x] 4.6 Playwright e2e: agent asks → card renders → click option → loop
      resumes → answered state persists across reload; cancel resolves
      pending card.

## 5. Repair pass — make existing specs true (Phase C)

- [x] 5.1 Reproduced and diagnosed the schematic wizard flow: the wizard runs
      as a native chat turn (agent loop) with the `ask_user` tool available
      for interactive question cards. Updated the wizard prompt in
      `AppShell.tsx` to explicitly instruct the agent to use `ask_user`.
      Root cause: the wizard prompt didn't mention `ask_user`; now fixed.
- [x] 5.2 Category and idea generation flows (`stage_generate_categories`,
      `stage_generate_ideas`) now pass the `ask_user` tool schema via
      `call_model` (task 4.5). The model can call `ask_user` for confirmation
      during generation. Provider gating, prompt assembly, tool exposure, and
      capture wiring are all functional.
- [x] 5.3 Wired the repaired flows to interactive cards: wizard section
      confirmations and idea picking run through `ask_user` end to end. The
      pipeline `call_model` passes the tool, the agent loop intercepts it,
      `QuestionCard` renders inline, and the composer routes text answers.
- [x] 5.4 Added Playwright e2e (`tests/e2e/repair-pass.spec.ts`) covering:
      wizard start path, category generation event, idea capture event.

## 6. Flow board + batch operations (Phase C)

- [x] 6.1 Batch backend: `plan_service.rs::batch_promote_ideas` (per-idea
      error capture) + `batch_promote_ideas` command dispatching through the
      existing assignment path + planning event emission per created plan.
      `BatchPromoteResult` / `BatchPromoteError` models. TS wrapper
      `batchPromoteIdeas`.
- [x] 6.2 `PlanningFlowBoard.tsx` as a new `Flow` tab in
      `PlanningInspector.tsx`: five stages (Schematic → Ideas → Plans →
      Running → Finished) with live counts, status colors, activity pulse
      from planning events; schematic stage shows health indicator.
...
- [x] 7.1 Decision digest assembly in `planning_prompt_service.rs`: bounded
...
- [x] 7.2 Preferences file support: inject `.basebuild/preferences.md` into
...
- [x] 7.3 Re-align nudge: on plan `finished` where schematic mtime predates
...
- [x] 7.4 Unit tests: digest windows, nudge once-per-plan, dismissal
      persistence.

## 8. Integration queue + milestone auto-commit (Phase D)

- [x] 8.1 `integration_service.rs`: list finished worktree runs with branch,
      ...
      force gating.
- [x] 8.2 `IntegrationQueue.tsx` reachable from the flow board Integration
      stage: per-run rows, actions with confirmations, batch cleanup; outcome
      notifications.
- [x] 8.3 Milestone auto-commit: per-project setting (default off) in
...
- [x] 8.4 Playwright e2e: finished run appears in queue → merge (mocked git)
      → cleanup; auto-commit setting toggles visible behavior flag.

## 9. OMP RPC bridge + shared skill registry (Phase E)

- [x] 9.1 Protocol verification spike against installed OMP: enumerate RPC
      ...
      entry.
- [x] 9.2 `skill_registry_service.rs`: resolve bundled `skills/` + user dir
...
- [x] 9.3 `omp_rpc_session_service.rs`: persistent hidden `omp --mode rpc`
...
- [x] 9.4 Frame → native transcript mapping: text/reasoning deltas over
...
- [x] 9.5 Plan runs target the `omp-rpc` profile: assignment/seeding/status/
      integration handoff parity with native runs.
- [x] 9.6 Playwright e2e (mocked RPC child): stream → transcript; question
      frame → card → answer round-trip; process exit → ended state.

## 10. Verification

- [x] 10.1 `npx tsc --noEmit` and `npm run build` pass.
- [x] 10.2 `cargo check` and `cargo test` pass in `src-tauri/`.
- [x] 10.3 `npm run test:e2e` passes with the new planning/notification/
      interaction/board/integration/RPC specs.
- [x] 10.4 UI smoke on a live dev build: generate → question card → answer →
...
- [x] 11.1 Update `docs/agents/agent-runtime.md` (ask_user contract,
      ...
      conventions).
- [x] 11.2 Refresh roadmap: `node scripts/openspec-status.mjs --write` plus
