...
- Add a permission/moderation layer for connector actions, including provider additions such as "OMP wants to add OpenAI subscription as a provider", command execution, file access, chat sync, external web UI/collaboration access, diagnostics, and analytics.
...
- `permission-provider-broker` - centralized prompts, rules, audits, and consent flows for provider imports, command/file access, chat sync, web UI embedding, diagnostics, and analytics.
